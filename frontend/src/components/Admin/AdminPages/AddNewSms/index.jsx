import DashsmsTable from "../../AdminComponents/AdminTable/DashsmsTable";
...
function AddNewSms() {
  ...
  return (
    <>
      <Outlet />
      <DashboardHeader title="Send Sms" desc="Manage your sms messages" />
      ...
      <PerfectScrollbar className="h-[calc(100vh-80px)] overflow-y-auto p-3 sm:p-5 xl:p-10">
        <div className="rounded-xl border border-gray-200/70 bg-white">
          ...
          {!publicationsQuery.isLoading && !publicationsQuery.isError && (
            <DashsmsTable data={publicationsQuery.data} />
          )}
        </div>
      </PerfectScrollbar>
    </>
